	...
	for epoch_num in range(epoch_length):
		...
		while True:
			try:
				...
				loss_class = model_classifier.evaluate([X, X2[:, sel_samples, :]], [Y1[:, sel_samples, :], Y2[:, sel_samples, :]])
				...
				progbar.update(iter_num, [('val_rpn_cls', np.mean(losses[:iter_num, 0])), ('val_rpn_regr', np.mean(losses[:iter_num, 1])),
