...
parser.add_option("-p_train", "--path_training", dest="train_path", help="Path to training data.")
parser.add_option("-p_val", "--path_validation", dest="val_path", help="Path to validation data.")
...
parser.add_option("--config_filename", dest="config_filename", help=
				"Location to read the metadata related to the pre-training.", default="config.pickle")
...
with open(options.config_filename, 'rb') as config:
    C = pickle.load(config)
...
# there may be an issue here, I think this is coded for BGR insted of RBG (coded for cv2)
...
# cannot do 90 degree rotations
data_gen = ImageDatagenerator(horizontal_flip=C.use_horizontal_flips, 
                                    vertical_flip=C.use_vertical_flips, 
                                    preprocessing_function=format_img(C=C))

data_gen_train = data_gen.flow_from_directory(options.)
data_gen_val = 
